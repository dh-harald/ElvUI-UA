-- QuestItemStarterDB: static item id lists for quest status the 1.12.1 API
-- cannot answer -- items that start a quest, keys that are quest items without
-- the "Quest" item type, and items wrongly carrying that type. Keys are item
-- ids as STRINGS. From real ElvUI-vanilla's !Compatibility addon
-- (!Compatibility/libs/QuestItemStarterDB.lua).
--
-- Changed from that copy: the minor version is a literal, because upstream
-- derived it with string.match, which Lua 5.0 does not have. It is kept above
-- upstream's 90001 so LibStub keeps this copy when both are loaded; the data is
-- unchanged.

local MAJOR_VERSION = "QuestItemStarterDB"
local MINOR_VERSION = 90002
