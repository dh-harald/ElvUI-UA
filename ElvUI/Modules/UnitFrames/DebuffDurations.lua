...
Durations['Cone of Cold']={[0]=8.0,} -- Cone of Cold


UF.DebuffDurations = Durations

local function GetMaxRank(effect)
	local max = 0
	local rank
	for rank in pairs(Durations[effect]) do
		if rank > max then max = rank end
	end
	return max
end

-- Always max rank -- no rank data available without the cast-hook layer
-- this pass deliberately doesn't port (see this file's own header
-- comment).
function UF:GetDebuffDuration(effect)
	if not effect or not Durations[effect] then return 0 end
	return Durations[effect][GetMaxRank(effect)] or 0
end

-- Hidden scan tooltip -- same established pattern as Core/Compat.lua's
-- own GetInventoryItemDurability (create once, reuse, ClearLines each
-- call) -- native UnitDebuff doesn't return a name on this API vintage,
-- only a tooltip actually shows it.
local scanTooltip

function UF:GetDebuffName(unit, index)
	if not scanTooltip then
		local ok
		ok, scanTooltip = pcall(CreateFrame, "GameTooltip", "ElvUFDebuffScanTooltip", nil, "GameTooltipTemplate")
		if not ok or not scanTooltip then return nil end
		scanTooltip:SetOwner(UIParent, "ANCHOR_NONE")
	end

	scanTooltip:ClearLines()
	local ok = pcall(scanTooltip.SetUnitDebuff, scanTooltip, unit, index)
	if not ok then return nil end

	local line = _G["ElvUFDebuffScanTooltipTextLeft1"]
	local name = line and line:GetText()

	-- THE mystery sliver, root-caused -- `SetUnitDebuff`
	-- shows the tooltip (same as any real GameTooltip:SetX call) but this
	-- function never hid it again afterward -- a real, permanently visible
	-- native GameTooltip (light background, rounded gold-trimmed border)
	-- sat on screen the instant any debuff's name was ever looked up, and
	-- never went away. Its width auto-fits the scanned debuff NAME text,
	-- which is exactly why it looked like "2 different sizes" (a longer
	-- name like "Hunter's Mark" vs. a shorter one like "Serpent Sting"
	-- producing 2 different tooltip widths), reproducible with any debuff,
	-- not one specific spell. The text needed has already been read by
	-- this point, so hiding it here has no effect on the returned name.
	pcall(scanTooltip.Hide, scanTooltip)

	return name
end

-- Stamp table -- keyed by unit NAME (not token), matching real
-- libdebuff's own approach: the same debuff on the same named target
-- stays tracked consistently even if the unit TOKEN pointing at it
-- changes (e.g. target vs mouseover on the same NPC). Deliberately no
-- unit-level sub-key (real libdebuff has one, to disambiguate same-
-- named-but-different-level mobs) -- accepted simplification, a name
-- collision here is rare and just means one debuff's timer restarts
-- early, not a wrong icon/duration.
UF.debuffStamps = UF.debuffStamps or {}

-- Returns seconds remaining, or nil if unknown/expired. Stamps the
-- CURRENT time as the start on first observation if this effect has no
-- known duration data and isn't already tracked (nil = "don't show a
-- timer", not "show 0").
function UF:GetDebuffTimeLeft(unit, effect)
	if not effect then return nil end

	local okName, unitName = pcall(UnitName, unit)
	if not okName or not unitName then return nil end

	local stamps = UF.debuffStamps[unitName]
	if not stamps then
		stamps = {}
		UF.debuffStamps[unitName] = stamps
	end

	local stamp = stamps[effect]
	if not stamp then
		local duration = UF:GetDebuffDuration(effect)
		if duration <= 0 then return nil end
		stamp = {start = GetTime(), duration = duration}
		stamps[effect] = stamp
	end

	local timeLeft = stamp.start + stamp.duration - GetTime()
	if timeLeft <= 0 then
		stamps[effect] = nil
		return nil
	end
...
-- Formats a cooldown-text string the same way Core/Cooldowns.lua does
-- (E:GetTimeInfo/E.TimeFormats) -- same visual language as the rest of
-- this project's own cooldown text, not a separate ad-hoc formatter.
