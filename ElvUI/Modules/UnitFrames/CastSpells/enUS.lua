...
-- Extracted verbatim from pfUI's spell database (env/locales_enUS.lua,
-- tables "spells" and "interrupts"; MIT License, Copyright (c)
-- 2016-2021 Eric Mauser (Shagu)).
-- Fallback: it only fills the tables when no locale file claimed them
-- earlier, so every client language without its own file (enGB, koKR,
-- zhTW, ...) gets the English names. The file matching the client locale
-- always overwrites both tables, so the result does not depend on the order
-- in which the client runs these files (Unreal Azeroth has been seen running
-- a nested loader XML's scripts ahead of scripts listed before it).

local E = unpack(ElvUI)
local UF = E.UnitFrames

if UF.castSpells then return end
